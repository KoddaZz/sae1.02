...
void lireFichierEntree(vector<string>& glaces, vector<vector<int>>& votes) {
    ...
    while(input.substr(0,3)!="fin"){
        ...
        if (input.substr(2, 21) == "voila la liste des joueurs")
        {
            cout<<"essai";
            lectureGlaces = false;
            lectureVotes = true;
        ...
        if (lectureGlaces==true) {
            ...
            for (size_t i = 0; i < input.size(); ++i) {
                if (isdigit(input[i])) {
                    ...
                    preferences.push_back(preference); // Ajuste pour un index de vecteur basé sur zéroccccccccc
